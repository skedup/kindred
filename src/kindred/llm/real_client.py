...
- 不得用“温柔、善良、勇敢”等常见美德替换原文的具体描述；原文没有的品质词不要自行概括；
- 优先保留跨处境稳定的性格、价值倾向、待人方式和表达气质；
- 保留会真实改变反应方式的对比面，不要把人物磨平成单一美德；
- 舍弃实现说明、系统职责、工具与行动建议、重复表达和一次性操作细节；
- 旧摘录只用于减少无意义措辞漂移；最终候选 SOUL 始终是唯一权威；
- 最多 500 个字符，不追求写满，不输出 markdown 标题或解释。
